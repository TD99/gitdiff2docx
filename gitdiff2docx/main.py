...
def main():
    """Main entry point for gitdiff2docx."""
    # The directory where this script is located (package directory)
    ...
    if config.get("open_after_creation", False):
        try:
            ...
            elif sys_module.platform == "darwin":  # macOS
                subprocess.run(["open", output_docx], check=False)
            else:  # Linux and other Unix-like systems
                subprocess.run(["xdg-open", output_docx], check=False)
        except Exception as e:
            # Silently ignore errors when opening file
            pass
